...
- **当前版本**：v0.5.0（开发中），待办与路线图见 [`TODO.md`](./TODO.md)
...
- 侧栏导航：图库（所有照片 / 图片 / 收藏夹 / 已隐藏）+ 媒体类型（视频 / 自拍 / 实况照片 / 截屏）+ 智能相簿 + 文件夹（含「最近打开」一键重开），各带实时计数；侧栏还可进入时光画廊与重复照片检测两个整页视图
- QuickLook 预览：图片支持滚轮**以光标为中心缩放**（双击在 1x / 2x 间切换）、旋转、带边界约束的拖拽平移、缩略图模糊占位淡入、相邻图片预加载；视频使用自定义播放器 —— 进度条含**已缓冲区间**与悬停时间提示、`±10 秒`快进快退、倍速（0.5×–2×）、画质（适应窗口 / 原始尺寸 1:1 可拖动查看）、音量与静音、循环、画中画、控制器全屏，缓冲时给出反馈，解码失败时说明具体原因（编码不支持 / 文件损坏 / 读取失败）并提供「在访达中打开」；图片与视频共用上一张 / 下一张、页码、收藏与**幻灯片自动播放**
...
- 侧栏在「图库分类 / 媒体类型 / 智能相簿 / 最近打开的文件夹」间切换，也可进入**时光画廊**（按年月回顾）与**重复照片检测**两个整页视图；
...
| 文件 / 目录 | 内容 | 说明 |
| --- | --- | --- |
| `config.json` | 收藏、隐藏、标签、时间修正、智能相簿、视频元数据、最近打开目录与全部视图偏好 | 临时文件 + rename 原子写入；解析失败自动备份为 `config.json.corrupt-<时间戳>` |
...
- **尚无**：撤销/重做（F2）、地图式按地点浏览（F3 / X4）、元数据写入（F4）、基础图片编辑（X3）、测试与 CI（X8）。
